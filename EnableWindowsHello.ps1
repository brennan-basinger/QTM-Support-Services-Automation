...
# region 2: Registry – Enable Windows Hello for Business and allow post-logon provisioning

Write-Host "Configuring registry to enable Windows Hello for Business and allow post-logon provisioning..."

# 2.1 Enable Windows Hello for Business (policy equivalent to 'Use Windows Hello for Business = Enabled')
$pfwKey = "HKLM:\SOFTWARE\Policies\Microsoft\PassportForWork"
New-Item -Path $pfwKey -Force | Out-Null

# Enabled = 1  → Windows Hello for Business enabled
New-ItemProperty -Path $pfwKey -Name "Enabled" -Value 1 -PropertyType DWord -Force | Out-Null

# DisablePostLogonProvisioning = 0 → allows the "set up a PIN / Windows Hello" prompt after logon
# You may omit this value entirely if you want the default behavior, but setting it to 0 is explicit.
New-ItemProperty -Path $pfwKey -Name "DisablePostLogonProvisioning" -Value 0 -PropertyType DWord -Force | Out-Null

Write-Host "  - PassportForWork: Enabled=1, DisablePostLogonProvisioning=0 set."
# endregion
